...
saveRDS(results, file = "output/result_sommer_row_col_random_optimized.RDS")
```
...
extract_residuals <- function(res) {
  resid <- residuals(res$model)
  data.frame(resid, trait = res$Trait, row.names = NULL)
}

# Função para extrair e criar um data frame a partir dos resíduos de cada modelo
extract_residuals <- function(res) {
  extract_res <- residuals(res$model)
  df <- data.frame(extract_res[, c(1:5, 8, 15)], trait = res$Trait)
  colnames(df)[6:7] <- c("fitted", "residuals")
  return(df)
}

fitted_residual_data <- map_dfr(results, extract_residuals)
```
...
varcomp %>%
  ...
  mutate(freq_relat = round(freq_relat * 100, 2)) %>%  select(1, 2, 7) %>%
...
varcomp %>%
  mutate(
    ...
    grp = fct_relevel(grp, c(
      "Row", "Col", "Year", "Clone", "Clone:Year", "Residual"
    )) # Coloca Residual como último
  ) %>%
  ggplot(aes(
    x = trait,
    y = VarComp,
...
```{r include=FALSE}
ggsave("output/varcomp_row_col.tiff",
       width = 16,
       height = 8)
```
...
Distribution of the best linear unbiased prediction (BLUP) 

```{r}
BLUPS %>%
  pivot_longer(8:9, names_to = "Method", values_to = "Values") %>%
  filter(Method == "BLUPS_mean") %>%
...
ggsave(
  "output/BLUPS_density_med_row_col.png",
  width = 12,
  height = 10,
  dpi = 300
)
...
############# SUPLEMMENTARY FIGURE 2 and 3 - residual distribution and their association against fitted values 


```{r}
# 1. Gráfico da distribuição dos resíduos
fitted_residual_data %>% 
...
ggsave(
  "output/Density_residual_row_col.tiff",
  width = 12,
  height = 8,
  dpi = 600
)
...
```{r}
# 2. Associação entre resíduos e valores ajustados
fitted_residual_data %>%
...
ggsave(
  "output/Residuals_vs_fitted_row_col.tiff",
  width = 12,
  height = 8,
  dpi = 600
)
...
#### Supplementary Table 2 - teste_LRT 
